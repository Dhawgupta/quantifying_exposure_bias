...
def quantify_exposure_bias_pretrained(output_dir: str,
    ...
    oracle_lm_model = AutoModelForCausalLM.from_pretrained(oracle_model)
    # oracle_model = AutoModelForCausalLM.from_pretrained(oracle_model)

    if cuda_device > -1:
        model.cuda()
        oracle_lm_model.cuda()
    model.eval()
    oracle_lm_model.eval()
    ...
    for sample_num, input_dict in enumerate(dataloader):
        ...
        if sample_num >= num_samples:
            break
        # why is this being generated by the actual models
        context_tokens = input_dict['input_ids'][:, :context_len]
        # input_dict.keys() = ['input_ids', 'attention_mask', 'labels']
        ...
        with torch.no_grad():
            ...
            #             temperature=sampling_temperature)
            # detokenized_oracle_predictions: List[str] = tokenizer.batch_decode(oracle_prediction,
        ...
        detokenized_targets: List[str] = tokenizer.batch_decode(oracle_prediction,
                                                            skip_special_tokens=True)
        

        # target_tokens = input_dict['input_ids']
        target_tokens = oracle_prediction
        ...
        # ALready made the context tokens above
        # context_tokens = input_dict['input_ids'][:, :context_len]
        ...
        # Get the logits for the generated setnence
